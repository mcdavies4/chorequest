...
export function OnboardingFlow({ onComplete }) {
  ...
  const validate = () => {
    ...
    if (step === 3) {
      kids.forEach(k => {
        if (!(selectedChores[k.id]?.length >= 1)) e[`chores${k.id}`] = 'Pick at least 1 chore'
      })
    }
    if (step === 4) {
      kids.forEach(k => {
  ...
  const toggleChore = (kidId, chore) => {
    setSelectedChores(prev => {
      ...
      return {
        ...prev,
        [kidId]: exists
          ? cur.filter(c => c.title !== chore.title)
          : [...cur, { ...chore, id: Date.now() + Math.random(), done: false, pending: false }],
      }
    })
  }
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#0f172a 0%,#1e293b 100%)', fontFamily: "'Nunito',sans-serif", padding: '24px 20px 40px' }}>
      ...
      {step > 0 && step < 5 && (
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
            {['You', 'Kids', 'Chores', 'Goals'].map((label, i) => (
              <div key={label} style={{ fontSize: 10, fontWeight: 800, color: step > i ? '#f59e0b' : '#334155' }}>
                {step > i ? '✓ ' : ''}{label}
              </div>
            ))}
          </div>
      ...
      {step === 0 && (
        <div style={{ textAlign: 'center', padding: '0 8px' }}>
          ...
          <div style={{ color: '#64748b', fontSize: 15, fontWeight: 600, lineHeight: 1.6, marginBottom: 32 }}>
          ...
          <button onClick={() => setStep(1)} style={{ width: '100%', background: 'linear-gradient(135deg,#f59e0b,#f97316)', border: 'none', borderRadius: 16, padding: 16, fontFamily: "'Fredoka One',cursive", fontSize: 22, color: 'white', cursor: 'pointer', boxShadow: '0 6px 24px #f59e0b55' }}>
            Set Up My Family 🚀
          </button>
      ...
      {step === 1 && (
        <div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input style={inp(errors.parentName)} value={parentName}
              onChange={e => { setParentName(e.target.value); setErrors(er => ({ ...er, parentName: null })) }}
              placeholder="e.g. Sarah" autoComplete="given-name" autoCorrect="off" autoCapitalize="words" />
            {errors.parentName && <div style={{ color: '#ef4444', fontSize: 12, fontWeight: 700, marginTop: 4 }}>{errors.parentName}</div>}
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={lbl}>Email Address</label>
            <input type="email" inputMode="email" style={inp(errors.parentEmail)} value={parentEmail}
              onChange={e => { setParentEmail(e.target.value); setErrors(er => ({ ...er, parentEmail: null })) }}
              placeholder="you@example.com" autoComplete="email" autoCorrect="off" autoCapitalize="none" />
            {errors.parentEmail && <div style={{ color: '#ef4444', fontSize: 12, fontWeight: 700, marginTop: 4 }}>{errors.parentEmail}</div>}
          </div>
          <div style={{ marginBottom: 8 }}>
            <label style={lbl}>Password</label>
            <input type="password" style={inp(errors.parentPw)} value={parentPw}
              onChange={e => { setParentPw(e.target.value); setErrors(er => ({ ...er, parentPw: null })) }}
        ...
        <div>
          ...
          {kids.length > 1 && (
            <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
              {kids.map((k, i) => (
                <button key={k.id} onClick={() => setActiveKidIdx(i)} style={{ flex: 1, background: activeKidIdx === i ? '#f59e0b' : '#1e293b', border: activeKidIdx === i ? 'none' : '1.5px solid #334155', borderRadius: 12, padding: '8px 4px', fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 13, color: activeKidIdx === i ? '#1e293b' : '#64748b', cursor: 'pointer' }}>
          ...
          <div style={{ background: '#1e293b', border: '1.5px solid #334155', borderRadius: 20, padding: 18, marginBottom: 14 }}>
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {KID_AVATARS.slice(0, 12).map(av => (
                  <button key={av} onClick={() => updateKid(activeKidIdx, 'avatar', av)} style={{ fontSize: 28, background: activeKid.avatar === av ? '#f59e0b22' : 'transparent', border: activeKid.avatar === av ? '2px solid #f59e0b' : '2px solid transparent', borderRadius: 12, padding: '4px 6px', cursor: 'pointer' }}>
                    {av}
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
              <div>
                ...
                <input style={{ ...inp(errors[`kid${activeKidIdx}name`]), padding: '11px 12px' }} value={activeKid.name}
                  onChange={e => { updateKid(activeKidIdx, 'name', e.target.value); setErrors(er => ({ ...er, [`kid${activeKidIdx}name`]: null })) }}
                  placeholder="First name" autoComplete="off" autoCorrect="off" autoCapitalize="words" />
              ...
              <div>
                ...
                <input type="number" inputMode="numeric" style={{ ...inp(errors[`kid${activeKidIdx}age`]), padding: '11px 12px' }} value={activeKid.age}
                  onChange={e => { updateKid(activeKidIdx, 'age', e.target.value); setErrors(er => ({ ...er, [`kid${activeKidIdx}age`]: null })) }}
                  placeholder="e.g. 9" min="3" max="17" />
            ...
            <div>
              ...
              <input type="password" inputMode="numeric" style={inp(errors[`kid${activeKidIdx}pw`])} value={activeKid.pw}
                onChange={e => { updateKid(activeKidIdx, 'pw', e.target.value); setErrors(er => ({ ...er, [`kid${activeKidIdx}pw`]: null })) }}
                placeholder="Min. 3 characters" autoComplete="new-password" />
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {kids.length < 4 && (
              <button onClick={() => { const id = Date.now(); setKids(prev => [...prev, { id, name: '', age: '', avatar: '🐼', pw: '' }]); setActiveKidIdx(kids.length) }} style={{ flex: 1, background: '#22c55e15', border: '1.5px solid #22c55e', borderRadius: 12, padding: 10, fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 13, color: '#22c55e', cursor: 'pointer' }}>
                + Add Another Kid
              </button>
            )}
            {kids.length > 1 && (
              <button onClick={() => { setKids(prev => prev.filter((_, i) => i !== activeKidIdx)); setActiveKidIdx(Math.max(0, activeKidIdx - 1)) }} style={{ background: '#ef444415', border: '1.5px solid #ef4444', borderRadius: 12, padding: '10px 16px', fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 13, color: '#ef4444', cursor: 'pointer' }}>
                Remove
              </button>
            )}
          </div>
        ...
        <div>
          ...
          {kids.length > 1 && (
            <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
              {kids.map((k, i) => (
                <button key={k.id} onClick={() => setActiveKidIdx(i)} style={{ flex: 1, background: activeKidIdx === i ? '#f59e0b' : '#1e293b', border: activeKidIdx === i ? 'none' : '1.5px solid #334155', borderRadius: 12, padding: '7px 4px', fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 12, color: activeKidIdx === i ? '#1e293b' : '#64748b', cursor: 'pointer' }}>
                  {k.avatar} {k.name}
                </button>
              ))}
            </div>
          )}
          <div style={{ fontWeight: 800, color: '#f59e0b', fontSize: 13, marginBottom: 10 }}>
            {activeKid.avatar} {activeKid.name}'s chores — {(selectedChores[activeKid.id] || []).length} selected
          </div>
            ...
            {SUGGESTED_CHORES.map(chore => {
              ...
              return (
                <div key={chore.title} onClick={() => { toggleChore(activeKid.id, chore); setErrors(er => ({ ...er, [`chores${activeKid.id}`]: null })) }} style={{ background: sel ? '#f59e0b15' : '#1e293b', border: sel ? '2px solid #f59e0b' : '1.5px solid #334155', borderRadius: 16, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', transition: 'all 0.18s' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, color: 'white', fontSize: 13 }}>{chore.title}</div>
                    <div style={{ fontSize: 11, color: '#f59e0b', fontWeight: 700 }}>🪙 ${chore.coins.toFixed(2)}</div>
                  </div>
                  <div style={{ width: 26, height: 26, borderRadius: '50%', background: sel ? '#f59e0b' : 'transparent', border: sel ? 'none' : '2px solid #334155', display: 'grid', placeItems: 'center', fontSize: 14, color: '#1e293b', flexShrink: 0 }}>
                    {sel ? '✓' : ''}
                  </div>
                </div>
              )
            ...
            {kids.map(k => (
              <div key={k.id} style={{ background: '#1e293b', border: '1.5px solid #334155', borderRadius: 20, padding: 16 }}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input style={inp(errors[`goalname${k.id}`])} value={goals[k.id]?.name || ''}
                    onChange={e => { setGoals(g => ({ ...g, [k.id]: { ...g[k.id], name: e.target.value } })); setErrors(er => ({ ...er, [`goalname${k.id}`]: null })) }}
                    placeholder="e.g. LEGO Set, Nintendo Switch..." autoComplete="off" autoCorrect="off" autoCapitalize="words" />
                ...
                <div>
                  ...
                  <input type="number" inputMode="decimal" min="1" step="1" style={inp(errors[`goaltarget${k.id}`])} value={goals[k.id]?.target || ''}
                    onChange={e => { setGoals(g => ({ ...g, [k.id]: { ...g[k.id], target: e.target.value } })); setErrors(er => ({ ...er, [`goaltarget${k.id}`]: null })) }}
                    placeholder="e.g. 25" />
      ...
      {step === 5 && (
        <div style={{ textAlign: 'center', padding: '20px 8px' }}>
            ...
            {kids.map(k => (
              <div key={k.id} style={{ background: '#1e293b', border: '1.5px solid #334155', borderRadius: 18, padding: '14px 16px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {(selectedChores[k.id] || []).map(c => (
                    <div key={c.title} style={{ background: '#0f172a', borderRadius: 99, padding: '3px 10px', fontSize: 11, fontWeight: 700, color: '#f59e0b' }}>
                      {c.icon} {c.title}
                    </div>
                  ))}
                </div>
          ...
          <button onClick={handleFinish} style={{ width: '100%', background: 'linear-gradient(135deg,#f59e0b,#f97316)', border: 'none', borderRadius: 16, padding: 16, fontFamily: "'Fredoka One',cursive", fontSize: 22, color: 'white', cursor: 'pointer', boxShadow: '0 6px 24px #f59e0b55' }}>
      ...
      {step > 0 && step < 5 && (
        <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
          <button onClick={back} style={{ flex: 1, background: 'transparent', border: '1.5px solid #334155', borderRadius: 14, padding: 13, fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 15, color: '#64748b', cursor: 'pointer' }}>
            ← Back
          </button>
          <button onClick={next} style={{ flex: 2, background: 'linear-gradient(135deg,#f59e0b,#f97316)', border: 'none', borderRadius: 14, padding: 13, fontFamily: "'Fredoka One',cursive", fontSize: 18, color: 'white', cursor: 'pointer', boxShadow: '0 4px 16px #f59e0b44' }}>
